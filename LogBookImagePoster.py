...
#./LogBookImagePoster.py -i XCS -w https://pswww/pcdsn/ws-auth -u xcsopr -e current

# import syntax:
# import -trim -frame -border -crop 972x1164+0+0 -window "Camera: XCS CAMERA 1" /tmp/2.jpg
#
###################################################
...
class LogBookImagePoster:
    ...
    def __init__(self, parent):
        ...
        self.descr.insert(0, sImageDesc)
        ...
        #self.canvas.grid(row=10,column=0,padx=5,pady=0,sticky=W+N+E+S)
        
        self.submit.configure(state=DISABLED)
        self.cancel.configure(state=DISABLED)
        
        #if sFnAttach != None:
        #  Timer(3,self.on_grab).start()
        root.after(0, self.on_grab)
        
    def on_grab(self):
        #self.f_jpeg = tempfile.NamedTemporaryFile(mode='r+b',suffix='.jpeg')
        self.f_jpeg = sFnAttach
        f_gif = tempfile.NamedTemporaryFile(mode='r+b',suffix='.gif').name
        print self.f_jpeg, f_gif
        #if( 0 == os.system("import -trim -frame -border %s; convert %s %s" % (self.f_jpeg, self.f_jpeg, f_gif))):
        if( 0 == os.system("convert %s %s" % (self.f_jpeg, f_gif))):
            photo = PhotoImage(file=f_gif)
...
""" % (progname)
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:], 'hi:e:w:u:p:a:t:d:')
    ...
    root = Tk()
    root.title("LogBookImagePoster")
    d = LogBookImagePoster( root )
    root.mainloop()
